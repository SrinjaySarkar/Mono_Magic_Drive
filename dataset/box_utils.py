...
def trans_boxes_to_views(bboxes, transforms, aug_matrixes=None, proj=True):
    ...
    Args:
        ...
        transforms (List[np.arrray]): each is n_camx4x4.
        aug_matrixes (List[np.array], optional): each is n_camx4x4. Defaults to None.
    ...
    coords = []
    # print("jibasidbs",len(transforms))
...
def trans_boxes_to_view(bboxes, transform, aug_matrix=None, proj=True):
    ...
    # print("BBOXS",len(bboxes))
    # print("transforms",transform.shape)
    # print("aug matrix",aug_matrix.shape)
    # print("proj",proj)
    
    # exit()

    bboxes_trans = box_center_shift(bboxes, (0.5, 0.5, 0.5))#center of nuscenes coord system
    trans = transform
    
    # print("lidar3d boxes passed to trans_box_to_View",bboxes_trans.corners.shape)
    # print("transform passed to trans_box_to_View",transform.shape)
    # print("aug matrix passed to trans_box_to_View",aug_matrix.shape)
    
    # transform the lidar2camera for augmented images.
