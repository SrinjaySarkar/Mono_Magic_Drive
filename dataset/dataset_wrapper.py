...
class FolderSetWrapper(torch.utils.data.DataLoader):
    ...
    def __getitem__(self,idx):
        ...
        mmdet3d_format={}
        mmdet3d_format["gt_mask"]=data["gt_masks_bev"]
        ...
        mmdet3d_format["liadr2camera"]=DataContainer(data["lidar2camera"])
        mmdet3d_format["img_aug_matrix"]=DataContainer(data["img_aug_matrix"])
        mmdet3d_format["metas"]=DataContainer(data["metas"])
        #special class
        gt_bboxes_3d=data["gt_bboxes_3d"][:,:7]
        mmdet3d_format["gt_bboxes_3d"]=DataContainer(LiDARInstance3DBoxes(gt_bboxes_3d,box_dim=gt_bboxes_3d.shape[-1],origin=(0.5,0.5,0)).convert_to(Box3DMode.LIDAR))
        ...
        camera2lidar[:,:3,3:]=torch.bmm(-camera2lidar[:,:3,:3],data["lidar2camera"][:,:3,3:])#rotate the translation vector
        mmdet3d_format["camera2lidar"]=DataContainer(camera2lidar)
        mmdet3d_format["lidar2image"]=DataContainer(torch.bmm(data["camera2lidar"],data["lidar2camera"]))
