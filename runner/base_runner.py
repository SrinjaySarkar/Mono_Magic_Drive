...
from diffusers import (AutoencoderKL,DDPMScheduler,UniPCMultistepScheduler,UNet2DConditionModel,)
...
from misc.common import (move_to,load_module,deepspeed_zero_init_disabled_context_manager)
from runner.base_validator import BaseValidator
from runner.utils import smart_param_count,append_dims,prepare_ckpt,resume_all_scheduler


class BaseRunner:
    def __init__(self,cfg,accelerator,train_set,val_set):
        self.cfg=cfg
        ...
        self.validator=BaseValidator(self.cfg,self.val_dataset,pipe_cls,pipe_param={"vae": self.vae,"text_encoder": self.text_encoder,"tokenizer": self.tokenizer})
    ...
    def _calculate_steps(self):
        ...
        self.num_update_steps_per_epoch=math.ceil(len(self.train_dataloader)/self.cfg.runner.gradient_accumulation_steps)
        # here the logic takes steps as higher priority. once set, will override
        # epochs param
        if self.overrode_max_train_steps:
            self.cfg.runner.max_train_steps=(self.cfg.runner.num_train_epochs * self.num_update_steps_per_epoch)
        else:
            # make sure steps and epochs are consistent
            self.cfg.runner.num_train_epochs=math.ceil(self.cfg.runner.max_train_steps/self.num_update_steps_per_epoch)

    def _init_trainable_models(self,cfg):
        model_cls=load_module(cfg.model.model_module)#magicdrive.networks.unet_addon_rawbox.BEVControlNetModel
        controlnet_param=OmegaConf.to_container(self.cfg.model.controlnet,resolve=True)
        self.controlnet=model_cls.from_unet(self.unet,**controlnet_param) #Instantiate BEVControlnet class from UNet2DConditionModel(pretrained ldm).

    def _set_dataset_loader(self):
        collate_fn_param={"tokenizer":self.tokenizer,"template":self.cfg.dataset.template,"bbox_mode":self.cfg.model.bbox_mode,"bbox_view_shared":self.cfg.model.bbox_view_shared,"bbox_drop_ratio":self.cfg.runner.bbox_drop_ratio,"bbox_drop_ratio": self.cfg.runner.bbox_drop_ratio,"bbox_add_num":self.cfg.runner.bbox_add_num}
        
        if self.train_dataset is not None:
            self.train_dataloader=torch.utils.data.DataLoader(self.train_dataset,shuffle=True,collate_fn=partial(collate_fn,is_train=True,**collate_fn_param),batch_size=self.cfg.runner.train_batch_size,num_workers=self.cfg.runner.num_workers,pin_memory=True,prefetch_factor=self.cfg.runner.prefetch_factor,persistent_workers=True)
        
        if self.val_dataset is not None:
            self.val_dataloader=torch.utils.data.DataLoader(self.val_dataset,shuffle=False,collate_fn=partial(collate_fn,is_train=False,**collate_fn_param),batch_size=self.cfg.runner.validation_batch_size,num_workers=self.cfg.runner.num_workers,prefetch_factor=self.cfg.runner.prefetch_factor)
        
    def _set_model_trainable_state(self,train=True):
        self.vae.requires_grad_(False) # frozen encoder for inputs
        self.text_encoder.requires_grad_(False) # frozen text encoder
        self.controlnet.train(train) # only finetune ldm init model
        self.unet.requires_grad_(False) # dont finetune the multi-view model do it later 
        ...
        if self.cfg.runner.enable_xformers_memory_efficient_attention:
            if is_xformers_available():
                ...
                if xformers_version==version.parse("0.0.16"):
                    logging.warn("xFormers 0.0.16 cannot be used for training in some GPUs. If you observe problems during training, please update xFormers to at least 0.0.17. See https://huggingface.co/docs/diffusers/main/en/optimization/xformers for more details.")
    ...
    def _set_gradient_checkpointing(self):
        if hasattr(self.cfg.runner.enable_unet_checkpointing, "__len__"):
            self.unet.enable_gradient_checkpointing(self.cfg.runner.enable_unet_checkpointing)
    ...
    def set_optimizer_schedule(self):
        ...
        # logging.info(f"[MultiviewRunner] add {param_count} params from unet to optimizer.")
    ...
    def _add_noise(self,latents,noise,timesteps):
        if self.cfg.runner.noise_offset:
            offset=self.cfg.runner.offset*append_dims(torch.randn(latents.shape[:2],device=latents.device),latents.ndim).type_As(latents)
        ...
        if timesteps.ndim==2:
            ...
            def bc2b(x):return x
            def b2bc(x):return x
        noisy_latents=self.noise_scheduler.add_noise(bc2b(latents),bc2b(noise),bc2b(timesteps))
    ...
    def _train_one_stop(self,batch):
        ...
        self.controlnet.train() #(controlnet+unet)
        with self.accelerator.accumulate(self.controlnet):
            ...
            timesteps,camera_param=camera_param, #b,n_cam,189
            ...
            return_dict=False,**batch["kwargs"])
            ...
            model_pred=self.unet(noisy_latents,# b*x,4,H/8, W/8
            timesteps.reshape(-1),  # b*n
            encoder_hidden_states=encoder_hidden_states_with_cam.to(
            dtype=self.weight_dtype), # b*n,len + 1,768
    ...
    def _validation(self,step):
        controlnet=self.accelerator.unwrap_model(self.controlnet)
        unet=self.accelerator.unwrap_model(self.unet)
        image_logs=self.validator.validate(controlnet,unet,self.accelerator.trackers,step,self.weight_dtype,self.accelerator.device)
    
    def run(self):
        # Train!
        total_batch_size=(self.cfg.runner.train_batch_size * self.accelerator.num_processes * self.cfg.runner.gradient_accumulation_steps)
        ...
        if self.cfg.resume_from_checkpoint:
            ...
            if path is None:
                self.accelerator.print(f"Checkpoint'{self.cfg.resume_from_checkpoint}' does not exist. Starting a new training run.")
                self.cfg.resume_from_checkpoint=None
                initial_global_step=0
            else:
                self.accelerator.print(f"Resuming from checkpoint {self.cfg.resume_from_checkpoint}")
                load_path=prepare_ckpt(self.cfg.resume_from_checkpoint,self.accelerator.is_local_main_process)
                ...
                initial_global_step=(global_step * self.cfg.runner.gradient_accumulation_steps)
        ...
        ## start train
        progress_bar=tqdm(range(0,self.cfg.runner.max_train_steps),initial=initial_global_step,desc="steps",disable=not self.accelerator.is_local_main_process,miniters=self.num_update_steps_per_epoch,maxinterval=self.cfg.runner.display_per_n_min*60)        
        
        image_logs=None
        logging.info(f"Starting from epoch {first_epoch} to {self.cfg.runner.num_train_epochs}")
            ...
            for step,batch in enumerate(self.train_dataloader):
                ...
                if self.accelerator.sync_gradients:
                    ...
                    if global_step % self.cfg.runner.checkpointing_steps==0:
                        ...
                        save_path=os.path.join(self.cfg.log_root,sub_dir_name)
                ...
                if self.cfg.runner.save_model_per_epoch is not None:
                    if epoch % self.cfg.runner.save_model_per_epoch == 0:
                        logging.info(f"Save at step {global_step},epoch {epoch}")
                        self.accelerator.wait_for_everyone()
                        sub_dir_name=f"weight-E{epoch}-S{global_step}"
                        self._save_model(os.path.join(self.cfg.log_root,sub_dir_name))
                self.accelerator.wait_for_everyone()  
                continue
            break  
        self.accelerator.wait_for_everyone()
